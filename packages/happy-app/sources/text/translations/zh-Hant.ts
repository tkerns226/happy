...
export const zhHant: TranslationStructure = {
    ...
    tools: {
        ...
        taskTools: {
            taskCount: ({ count }: { count: number }) => `${count}個任務`,
            taskId: ({ id }: { id: string }) => `任務 #${id}`,
            updateStatus: ({ id, status }: { id: string; status: string }) => `任務 #${id} → ${status}`,
            owner: ({ name }: { name: string }) => `負責人: ${name}`,
            blockedBy: ({ ids }: { ids: string }) => `被阻擋: #${ids}`,
            stopTask: ({ id }: { id: string }) => `停止任務 #${id}`,
            taskOutput: ({ id }: { id: string }) => `任務 #${id} 的輸出`,
        },
        teamTools: {
            messageTo: ({ name }: { name: string }) => `傳送給 ${name}`,
            broadcast: '廣播給全部',
            shutdownRequest: ({ name }: { name: string }) => `關閉請求傳送給 ${name}`,
        },
        desc: {
    ...
    claude: {
        ...
        permissions: {
            ...
            approvePlan: '批准計畫',
            approveAllowEdits: '批准 + 允許編輯',
            approveFullAccess: '批准 + 完全存取',
            rejectPlan: '拒絕計畫',
        }
    },
