...
export const it: TranslationStructure = {
    ...
    tools: {
        ...
        taskTools: {
            taskCount: ({ count }: { count: number }) => `${count} attività`,
            taskId: ({ id }: { id: string }) => `Attività #${id}`,
            updateStatus: ({ id, status }: { id: string; status: string }) => `Attività #${id} → ${status}`,
            owner: ({ name }: { name: string }) => `Proprietario: ${name}`,
            blockedBy: ({ ids }: { ids: string }) => `Bloccato da: #${ids}`,
            stopTask: ({ id }: { id: string }) => `Ferma attività #${id}`,
            taskOutput: ({ id }: { id: string }) => `Output attività #${id}`,
        },
        teamTools: {
            messageTo: ({ name }: { name: string }) => `Messaggio a ${name}`,
            broadcast: 'Trasmissione a tutti',
            shutdownRequest: ({ name }: { name: string }) => `Richiesta di arresto a ${name}`,
        },
        multiEdit: {
    ...
    claude: {
        ...
        permissions: {
            ...
            approveAllowEdits: 'Approva + consenti modifiche',
            approveFullAccess: 'Approva + accesso completo',
            rejectPlan: 'Rifiuta piano',
        }
    },
