...
export const ja: TranslationStructure = {
    ...
    tools: {
        ...
        taskTools: {
            taskCount: ({ count }: { count: number }) => `${count}件のタスク`,
            taskId: ({ id }: { id: string }) => `タスク #${id}`,
            updateStatus: ({ id, status }: { id: string; status: string }) => `タスク #${id} → ${status}`,
            owner: ({ name }: { name: string }) => `担当者: ${name}`,
            blockedBy: ({ ids }: { ids: string }) => `ブロック: #${ids}`,
            stopTask: ({ id }: { id: string }) => `タスク #${id} を停止`,
            taskOutput: ({ id }: { id: string }) => `タスク #${id} の出力`,
        },
        teamTools: {
            messageTo: ({ name }: { name: string }) => `${name} へのメッセージ`,
            broadcast: '全員へ送信',
            shutdownRequest: ({ name }: { name: string }) => `${name} へのシャットダウン要求`,
        },
        multiEdit: {
    ...
    claude: {
        ...
        permissions: {
            ...
            approvePlan: 'プランを承認',
            approveAllowEdits: '承認 + 編集を許可',
            approveFullAccess: '承認 + フルアクセス',
            rejectPlan: 'プランを却下',
        }
    },
