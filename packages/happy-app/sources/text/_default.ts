...
export const en = {
    ...
    tools: {
        ...
        taskTools: {
            taskCount: ({ count }: { count: number }) => `${count} ${plural({ count, singular: 'task', plural: 'tasks' })}`,
            taskId: ({ id }: { id: string }) => `Task #${id}`,
            updateStatus: ({ id, status }: { id: string; status: string }) => `Task #${id} → ${status}`,
            owner: ({ name }: { name: string }) => `Owner: ${name}`,
            blockedBy: ({ ids }: { ids: string }) => `Blocked by: #${ids}`,
            stopTask: ({ id }: { id: string }) => `Stop task #${id}`,
            taskOutput: ({ id }: { id: string }) => `Output of task #${id}`,
        },
        teamTools: {
            messageTo: ({ name }: { name: string }) => `Message to ${name}`,
            broadcast: 'Broadcast to all',
            shutdownRequest: ({ name }: { name: string }) => `Shutdown request to ${name}`,
        },
        desc: {
    ...
    claude: {
        ...
        permissions: {
            ...
            approveAllowEdits: 'Approve + allow edits',
            approveFullAccess: 'Approve + full access',
